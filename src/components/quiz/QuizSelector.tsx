...
export const QuizSelector: React.FC<QuizSelectorProps> = ({
  ...
  return <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="max-w-2xl mx-auto">
        <ProductSelector value={selectedProductId} onValueChange={onProductChange} showPreview={false} placeholder="Selecione um produto para contextualizar o quiz" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {quizOptions.map(option => <Card key={option.id} className={`${option.bgColor} border-2 hover:scale-105 transition-all duration-300 cursor-pointer group rounded-2xl`} onClick={() => handleQuizSelect(option.id)}>

...
export function CTASection() {
  ...
  return (
    <section className="py-32 px-4 relative overflow-hidden bg-gradient-to-r from-[#3B82F6] via-[#2563EB] to-[#1E40AF]">
      <div className="absolute inset-0 bg-[#121212]/20" />
      
      {/* Floating shapes */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-32 -left-32 w-64 h-64 bg-[#FFFFFF]/10 rounded-full blur-3xl animate-float" />
        <div className="absolute -bottom-32 -right-32 w-64 h-64 bg-[#FFFFFF]/10 rounded-full blur-3xl animate-float animate-stagger-3" />
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#FFFFFF]/5 rounded-full blur-3xl animate-float animate-stagger-2" />
      </div>

      <div className="relative max-w-6xl mx-auto text-center animate-fade-in-up">
        <h2 className="text-5xl md:text-7xl font-bold text-[#FFFFFF] mb-8 leading-tight animate-fade-in-up">
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-12 animate-fade-in-up animate-stagger-2">
          {urgencyFeatures.map((feature, index) => (
            <div key={index} className="flex items-center gap-2 bg-[#FFFFFF]/10 backdrop-blur-sm rounded-2xl px-6 py-3">
              <feature.icon className="w-5 h-5 text-[#FFFFFF]" />
              <span className="text-[#FFFFFF] font-medium">{feature.text}</span>
            </div>
          ))}
        </div>
